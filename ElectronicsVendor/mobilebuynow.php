...
if(isset($_SESSION['id']))
{
	...
	$sql="select * from registration,mobile where modelno='$model' and cid=$id";
	$res=$con->query($sql);
	$row=$res->fetch_array();
}
else
...
<tr><td>Pincode : </td><td><input type='text' placeholder="Enter pincode" name='n3'></td></tr>
...
<tr><td>Price : </td><td><input type='text' name='n7' value="<?php echo $row[12] ?>" readonly></td></tr>
...
if(isset($_POST['btn']))
{
	...
	$total=$row[12]*$qty;
	$qty1=$row[43]-$qty;
	$sql1="update mobile set qty=$qty1 where modelno='$model'";
	$con->query($sql1);
	$ono=rand(1,99999);
