...
if(isset($_SESSION['id']))
{
	...
	$sql="select * from registration,refrigerator where modelname='$model' and cid=$id";
	$res=$con->query($sql);
	$row=$res->fetch_array();
}
else
...
<tr><td>Model Name : </td><td><input type='text' name='n4' value="<?php echo $model ?>" readonly></td></tr>
<tr><td>Model No. : </td><td><input type='text' name='n6' value="<?php echo $row[10] ?>" readonly></td></tr>
<tr><td>Price : </td><td><input type='text' name='n7' value="<?php echo $row[21] ?>" readonly></td></tr>
...
if(isset($_POST['btn']))
{
	...
	$total=$row[21]*$qty;
	$qty1=$row[22]-$qty;
	$sql1="update refrigerator set qty=$qty1 where modelname='$model'";
	$con->query($sql1);
	$ono=rand(1,99999);
	$sql2="insert into orders values(null,$ono,$cid,'$email','$address','$pin','$modelname','$modelno',$price,$qty,$total,'null')";
	$con->query($sql2);
	echo "<script>window.location.href='ordersummary.php?model=$row[9]'</script>";
}
?>