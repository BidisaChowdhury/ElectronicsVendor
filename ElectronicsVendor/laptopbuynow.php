...
if(isset($_SESSION['id']))
{
	...
	$sql="select * from registration,laptop where modelno='$model' and cid=$id";
	$res=$con->query($sql);
	$row=$res->fetch_array();
}
else
...
<tr><td>Model Name : </td><td><input type='text' name='n4' value="<?php echo $row[9] ?>" readonly></td></tr>
<tr><td>Model No. : </td><td><input type='text' name='n6' value="<?php echo $model ?>" readonly></td></tr>
<tr><td>Price : </td><td><input type='text' name='n7' value="<?php echo $row[27] ?>" readonly></td></tr>
...
if(isset($_POST['btn']))
{
	...
	$total=$row[27]*$qty;
	$qty1=$row[28]-$qty;
	$sql1="update laptop set qty=$qty1 where modelno='$model'";
	$con->query($sql1);
	$ono=rand(1,99999);
