...
function fy(str)
{
		...
		else
		{
			...
			xmlhttp.open("GET", "viewmobile.php?q=tv", true);
			xmlhttp.send();
		}
...
<p style="padding-top:10px">
	...
	<nav>
		...
		<input type='radio' name='n11' onChange='fz(this.value)' value='41'> Less than 20,000 <br>
		<input type='radio' name='n11' onChange='fz(this.value)' value='42'> More than 20,000 <br>
		</div><br>
		<p style="padding-left:4px;background-color:#000099;color:#ffffff;font-size:20px"> Sort By Screen Size </p>
		<div style="box-shadow: 1px 2px 2px 2px #d6d6c2;width:190px;height:100px">
		<input type='radio' name='n11' onChange='fz(this.value)' value='133'> Below 32 inch <br>
		<input type='radio' name='n11' onChange='fz(this.value)' value='131'> 32 inch <br>
		<input type='radio' name='n11' onChange='fz(this.value)' value='132'> 39-43 inch <br>
		<input type='radio' name='n11' onChange='fz(this.value)' value='134'> More than 43 inch <br>
		</div>
	</nav>
<article>
...
<?php
	$con=new MySQLi("localhost","root","","electronics");
	$sql="select * from tv where id between 1 and 6 order by modelname";
	$res=$con->query($sql);
	echo "<ul style='list-style-type:none;padding-left:35px'>";
	while($row=$res->fetch_array())
	{
		echo "<li><div class='gallery1'><a href='tvspecification.php?id=$row[0]'><img src='".$row[4]."'><div class='desc1'><p style='font-weight:900;font-size:14px'>".$row[2]."</p><p style='color:#006600;font-weight:600;font-size:12px'><strike>&#8377;".$row[9]."</strike></p><p style='font-weight:600;font-size:12px;color:#5c8a8a'>NOW &#8377;" .$row[10]."</p></div></a></div></li>";
    }
	echo "</ul>";
?>
</div>
