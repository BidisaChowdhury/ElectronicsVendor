...
if(isset($_SESSION['id']))
{
	...
	$sql="select * from registration,ac where modelname='$model' and cid=$id";
	$res=$con->query($sql);
	$row=$res->fetch_array();
}
else
...
<tr><td>Brand : </td><td><input type='text' name='n6' value="<?php echo $row[10] ?>" readonly></td></tr>
...
if(isset($_POST['btn']))
{
	...
	$total=$row[18]*$qty;
	$qty1=$row[19]-$qty;
	$sql1="update ac set qty=$qty1 where modelname='$model'";
	$con->query($sql1);
	$ono=rand(1,99999);
